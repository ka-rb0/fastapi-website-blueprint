...
node comes from the digest-pinned FROM node:<major>-slim line in the
consolidated Dockerfile (Dependabot bumps the digest, only a human bumps the
major). This test turns that Dockerfile's "must match engines.node" comment
into an enforced invariant. Pure file check - no server needed.
"""
...
def test_dev_image_node_major_matches_engines() -> None:
    """The Dockerfile's FROM node:<major> pin mirrors engines.node."""
    ...
    assert match, "FROM node:<major>-slim@sha256:... not found in Dockerfile"
